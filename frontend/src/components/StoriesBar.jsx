...
const StoriesBar = () => {
    ...
    const fetchStories = async () => {
        try {
            const res = await axios.get('http://localhost:8000/api/v1/story', { withCredentials: true });
            if(res.data.success){
    ...
    const handleUploadStory = async (e) => {
        ...
        try {
            const res = await axios.post('http://localhost:8000/api/v1/story', formData, {
    ...
    const handleViewStory = async (story) => {
        ...
        if(!story.views.includes(user?._id)){
            try {
                await axios.post(`http://localhost:8000/api/v1/story/${story._id}/view`, {}, { withCredentials: true });
